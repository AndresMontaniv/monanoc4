...
io.on('connection', (socket) => {
  ...
  socket.on('login', async ({ name, room }) =>  {
      ...
      const responseUser = await axios.get('http://localhost:3000/api/user/'+name);
      ...
      const responseProject = await axios.get('http://localhost:3000/api/cargar-diagrama/'+room);
  ...
  socket.on('save_component', async (  data ) => {
    ...
    if(findRoom(data.room)) {
        ...
        // TODO: CAMBIAR A LA URL DE MI API
        const response =  await axios.put('http://localhost:3000/api/guardar-diagrama/'+data.room, body);
  ...
  socket.on('auto_save', async (  data ) => {
...
server.listen( 9091);
console.log('servidor socket conectado en el puerto 9091');
module.exports = {
  sockets,
  server,
  io
